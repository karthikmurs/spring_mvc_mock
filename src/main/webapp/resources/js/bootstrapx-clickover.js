...
!function($) {
  ...
  Clickover.prototype = $.extend({}, $.fn.popover.Constructor.prototype, {
    ...
    , clickery: function(e) {
      ...
      if ( this.isShown() ) {
        var that = this;
        var cbInEditMode = this.options.cbInEditMode;
        var onClickOutside = this.options.onClickOutside;

        // close on global request, exclude clicks inside clickover
        this.options.global_close &&
          $('body').on( this.attr.click_event_ns, function(e) {
        	if( !cbInEditMode || (cbInEditMode && !cbInEditMode()) ){
        		if ( !that.tip().has(e.target).length ) { that.clickery(); }
        	}else{
        		// notify when user has clicked outside of the clickover
        		var len = that.tip().has(e.target).length;
                if( !len && onClickOutside ){
                	onClickOutside(this);
                }
        	}
          });
    	...
    	// fixed bug where handler is not unregistered properly
    	this.tip().off('click');
